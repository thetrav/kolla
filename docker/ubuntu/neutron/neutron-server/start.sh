...
check_required_vars KEYSTONE_ADMIN_TOKEN KEYSTONE_ADMIN_SERVICE_HOST KEYSTONE_AUTH_PROTOCOL \
  ...
  ADMIN_TENANT_NAME NEUTRON_SERVER_SERVICE_HOST PUBLIC_IP NEUTRON_DB_PASSWORD
check_for_keystone
check_for_db
...
crudini --set /etc/neutron/neutron.conf DEFAULT nova_url "http://${NOVA_API_SERVICE_HOST}:8774/v2"
crudini --set /etc/neutron/neutron.conf DEFAULT nova_admin_auth_url "http://${KEYSTONE_ADMIN_SERVICE_HOST}:35357/v2.0"
crudini --set /etc/neutron/neutron.conf DEFAULT nova_region_name "RegionOne"
crudini --set /etc/neutron/neutron.conf DEFAULT nova_admin_username "${NOVA_KEYSTONE_USER}"
crudini --set /etc/neutron/neutron.conf DEFAULT nova_admin_tenant_id "$(keystone tenant-list | grep $ADMIN_TENANT_NAME | awk '{print $2;}')"
crudini --set /etc/neutron/neutron.conf DEFAULT nova_admin_password "${NOVA_KEYSTONE_PASSWORD}"
